  ...
  async getCourseTeachers(courseId) {
    try {
      ...
      const response = await this.classroom.courses.teachers.list({
        courseId: courseId,
        fields: 'teachers(userId,profile(name,emailAddress,photoUrl))'
      });

      // Map to simplified format with name and email
      ...
      console.log(`Fetched ${teachers.length} teachers for course ${courseId}`);
      return teachers;
    } catch (error) {
      ...
      if (material.driveFile) {
        ...
        try {
          ...
          if (fileData.supported) {
            ...
            const isPermissionError = fileData.error && (
              fileData.error.includes('permission') || 
              fileData.error.includes('insufficient') ||
              fileData.error.includes('authentication scopes')
            );
            
            if (isPermissionError) {
              console.log(`DEBUG: Marking ${material.driveFile.driveFile.title} as requiring Drive access due to permission error`);
              return {
                type: 'driveFile',
                title: material.driveFile.driveFile.title,
            ...
            // For other unsupported file types, just mark as not downloadable
            return {
              type: 'driveFile',
              title: material.driveFile.driveFile.title,
              driveFileId: material.driveFile.driveFile.id,
          ...
          // If it's a permission error or Drive isn't initialized, mark as requiring Drive access
          if (!this.drive || 
              (error.message && (
              ...
              error.code === 403) {
            console.log(`DEBUG: Marking ${material.driveFile.driveFile.title} as requiring Drive access`);
            return {
              type: 'driveFile',
              title: material.driveFile.driveFile.title,
              driveFileId: material.driveFile.driveFile.id,
